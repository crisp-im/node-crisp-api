...
Crisp.prototype = {
  ...
  on : function(event, callback) {
    ...
    var guardError = this._guardCanBindEvent(event);
        ...
        self._brokerScheduler = setTimeout(function() {
          switch (rtmMode) {
            ...
            case Crisp.RTM_MODES.WebHooks: {
              // Prepare loopback now
              self._prepareLoopback()
                .then(resolve)
                .catch(reject);
  ...
  /**
   * Prepares loopback (used for Web Hooks)
   * @memberof Crisp
   * @private
   * @method _prepareLoopback
   */
  _prepareLoopback : function() {
  ...
  _emitAuthenticateSocket : function() {
    var auth        = this.auth;
    var tier        = this._tier;
    var boundEvents = Object.keys(this._boundEvents);
  ...
  /**
   * Guards the binding of an event
   * @memberof Crisp
   * @private
   * @method _guardCanBindEvent
   * @param {string} event
   */
  _guardCanBindEvent : function(event) {
    switch (this._rtm.mode) {
      case Crisp.RTM_MODES.WebSockets: {
        if (this._socket) {
          return new Error(
            "[Crisp] guardCanBindEvent: socket is already bound, please "  +
              "bind event earlier on: '" + event + "'"
          );
        }
      ...
      case Crisp.RTM_MODES.WebHooks: {
        if (this._loopback) {
          return new Error(
            "[Crisp] guardCanBindEvent: hooks are already bound, please "  +
              "bind event earlier on: '" + event + "'"
          );
        }
